...
var CalendarFetcher = function(url, reloadInterval, excludedEvents, maximumEntries, maximumNumberOfDays, auth, hideCompleted) {
	var self = this;

	var reloadTimer = null;
	var events = [];
	var showCompleted = !hideCompleted;
	...
	var fetchCalendar = function() {
		...
		ical.fromURL(url, opts, function(err, data) {
			...
			var eventDate = function(event, time) {
				var theDate = null;
				if (typeof(event[time]) === 'string' && event[time].length === 8) {
					theDate = moment(event[time], "YYYYMMDD")
				} else {
					theDate = moment(new Date(event[time]));
					if (!theDate.isValid()) {
						theDate = moment(event[time]);
					}
				}
				return theDate; 
			};
			...
			for (var e in data) {
				...
				if (event.type === "VTODO") {
					...
					var completed = (event.status && event.status === 'COMPLETED') ? true : false;
					var progress = event.completion ? parseInt(event.completion) : 0;
					...
					if (typeof event.due !== "undefined") {
						// console.log(' **** event.due: ' + event.due);
						// console.log(' **** event.due.val: ' + event.due.val);
						due = eventDate(event.due, "val");
					}
					var relatedTo = undefined;
					if (event['related-to'] !== "undefined") {
						relatedTo = event['related-to'];
					}

					// console.log(' **** relatedTo: ' + relatedTo);
						
					if (typeof(event.start) === 'string' && event.start.length === 8) {
					...
					if (typeof event.rrule !== "undefined" && event.rrule !== null) {
						// console.log('event has recurring rule...', JSON.stringify(event));
						...
						if (!showCompleted && completed) {
							// console.log("Completed events should not return.  So skip: " + title);
							continue;
						} else {
			...
			// console.log(' ');
			// console.log(' ');
			// console.log('****** Before tree', newEvents);

			// Loop through the array and copy each event into the subtask of its parent.
			// this should create a tree for us.
			newEvents.forEach( evnt => {
				if (evnt.relatedTo !== undefined) {
					var parentEvnt = newEvents.find( subEvnt => subEvnt.uid === evnt.relatedTo);
					if (parentEvnt) {
						parentEvnt.subtasks.push(evnt);
					}
				}
			});
			// console.log(' ');
			// console.log(' ');
			// console.log('====== After tree', newEvents);
			// Now filter the array to only show the parent events
			
			var parentEvents = newEvents.filter(evnt => evnt.relatedTo === undefined);
			newEvents = parentEvents;
			// console.log(' ');
			// console.log(' ');
			// console.log(' ');
			// console.log('>>>>>> Only parents', newEvents);
